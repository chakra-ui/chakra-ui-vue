...
> In the example below, the `_check-circle` is an internal icon provided by Chakra and the `cog` icon is from [Font Awesome](https://fontawesome). Learn more about using custom icons in chakra <nuxt-link to="/icon">here</nuxt-link>
