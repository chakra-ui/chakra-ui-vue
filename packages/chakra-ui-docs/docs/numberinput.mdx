...
- `CNumberInputStepper`: The wrapper for the input's stepper buttons.
...
> mobile specific implementation soon depending on the increase use cases.
...
Just like the `CInput` component, you can pass the `size` prop to change the size
of the input.
...
| ---------- | --------- | ------------------------------------------------------------------------------------------------------------ |
| `@change`  | `number`   | The event emitted when the `CNumberInput` value changes. It emits the current value as teh payload          |
