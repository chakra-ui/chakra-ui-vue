...
`CAlertIcon` composes `CIcon` and changes the icon based on the `status` prop. It will also change the icon based on the value of the `name` prop only if it is passed.
