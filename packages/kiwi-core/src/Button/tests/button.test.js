...
it('should display button with left icon', () => {
  const { container, html } = renderComponent({ template: '<Button leftIcon="email">Email</Button>' })
  expect(container.querySelector('button > svg')).toBeInTheDocument()
  expect(html()).toMatchSnapshot()
})

it('should display button with right icon', () => {
  const { container, html } = renderComponent({ template: '<Button rightIcon="email">Email</Button>' })
  expect(container.querySelector('button > svg')).toBeInTheDocument()
  expect(html()).toMatchSnapshot()
})
